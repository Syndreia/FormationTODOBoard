...
class TodoListWindow:
    ...
    def reload(self):
        ...
        for todoEntity in todoEntities:
            ...
            self.tree.insert("", tk.END, values=(todoEntity.id, todoEntity.label, done), tags=('task',))
            self.tree.tag_configure('task', foreground=configurationEntity.todoBackgroundColor)
    ...
    def show_configuration(self):
        ...
        config_window.protocol("WM_DELETE_WINDOW", lambda: self.__close_window(config_window, self.reload))
    ...
    def delete_todo(self):
        index = self.tree.selection()
        if index:
            answer = askokcancel("Supprimer", "Voulez-vous supprimer la ligne sélectionnée ?");
            if answer:
